...
?>
<div id="template-wrap" class="load <?php if ( $editable && !!get_user_setting( 'mymailshowmodules', 1 ) && !empty( $modules ) ) {
	echo ' show-modules';
}
...
?>
		...
		<div id="clickmap-stats">
			...
			<p><?php esc_html_e( 'Clicks', 'mymail' );?>: <strong class="clicks">0</strong><br><?php esc_html_e( 'Total', 'mymail' );?>: <strong class="total">0</strong></p>
		</div>
...
?>
	...
	<div id="plain-text-wrap">
		<?php $autoplaintext = !isset( $this->post_data['autoplaintext'] ) || $this->post_data['autoplaintext']?>
			<p><label><input type="checkbox" id="plaintext" name="mymail_data[autoplaintext]" value="1" <?php checked( $autoplaintext );?>> <?php esc_html_e( 'Create the plain text version based on the HTML version of the campaign', 'mymail' );?></label> <a class="alignright button button-small button-primary"><?php esc_html_e( 'get text from HTML version', 'mymail' );?></a></p>

			<textarea id="excerpt" name="excerpt" class="<?php if ( $autoplaintext ) {
	echo ' disabled';
}
...
?>
			<div id="module-selector">
				<a class="toggle-modules mymail-btn mymail-icon" title="<?php esc_html_e( 'Modules', 'mymail' )?>"></a>
				<div class="inner">
					<ul>
	...
	echo '<li data-id="' . $i . '" draggable="true"><a class="mymail-btn addmodule ' . ( $has_screenshots ? 'has-screenshot" style="background-image:url(\'' . $screenshot_modules_folder_uri . $screenshots[$i] . '\');height:' . ( ceil( $has_screenshots[1] / 2 ) + 6 ) . 'px;' : '' ) . '" title="' . esc_attr( sprintf( __( 'Click to add %s', 'mymail' ), '"' . $module . '"' ) ) . '" data-id="' . $i . '">' . esc_html( $module ) . '</a></li>';
}?>
					</ul>
				</div>
			</div>
...
<div id="mymail_campaign_preview" style="display:none;">
	<div class="device-wrap">
		...
		<p class="device-info"><?php esc_html_e( 'Your email may look different on mobile devices', 'mymail' );?></p>
		</div>
	</div>
</div>
