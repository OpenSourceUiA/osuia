...
class MyMailCampaigns {
	...
	public function init() {
		...
		add_action( 'mymail_auto_post_thumbnail', array( &$this, 'get_post_thumbnail' ), 10, 1 );
	...
	public function autoresponder_hook( $campaign_id, $subscriber_ids = null ) {
		...
		if ( !$meta['active'] || $meta['autoresponder']['action'] != 'mymail_autoresponder_hook' ) {
			return;
		}
	...
	public function meta_boxes() {
		...
		add_meta_box( 'mymail_receivers', __( 'Receivers', 'mymail' ), array( &$this, 'newsletter_receivers' ), 'newsletter', 'side', 'high' );
		add_meta_box( 'mymail_options', __( 'Options', 'mymail' ), array( &$this, 'newsletter_options' ), 'newsletter', 'side', 'high' );
	...
	public function columns_content( $column ) {
		...
		case "status":
			...
			if ( !in_array( $post->post_status, array( 'pending', 'auto-draft' ) ) ) {
				...
				case 'queued':
					...
					printf( __( 'starts in %s', 'mymail' ), ( $timestamp - $now > 60 ) ? human_time_diff( $timestamp ) : __( 'less than a minute', 'mymail' ) );
				...
				case "autoresponder":
					...
					if ( 'mymail_autoresponder_timebased' == $autoresponder['action'] ) {
						...
						printf( __( 'send every %1$s %2$s', 'mymail' ), '<strong>' . $autoresponder['interval'] . '</strong>', '<strong>' . $time_frame_names[$autoresponder['time_frame']] . '</strong>' );
						if ( $meta['timestamp'] ) {
						...
						if ( count( $autoresponder['weekdays'] ) < 7 ) {
							...
							printf( __( 'send every %1$s %2$s', 'mymail' ), '<strong>' . $autoresponder['useramount'] . '</strong>', '<strong>' . $time_frame_names[$autoresponder['userunit']] . '</strong>' );

							echo ' ' . sprintf( __( 'based on the users %1$s value', 'mymail' ), ' <strong>' . ( isset( $datefields[$autoresponder['uservalue']] ) ? $datefields[$autoresponder['uservalue']]['name'] : $autoresponder['uservalue'] ) . '</strong>' );
							...
							if ( $campaign = $this->get( $post->post_parent ) ) {
								...
								printf( __( 'send %1$s %2$s %3$s', 'mymail' ), ( $autoresponder['amount'] ? '<strong>' . $autoresponder['amount'] . '</strong> ' . $mymail_autoresponder_info['units'][$autoresponder['unit']] : __( 'immediately', 'mymail' ) ), __( 'after', 'mymail' ), ' <strong><a href="post.php?post=' . $campaign->ID . '&action=edit">' . $campaign->post_title . '</a></strong> ' . $types[$autoresponder['followup_action']] );
						...
						printf( __( 'send %1$s %2$s %3$s', 'mymail' ), ( $autoresponder['amount'] ? '<strong>' . $autoresponder['amount'] . '</strong> ' . $mymail_autoresponder_info['units'][$autoresponder['unit']] : __( 'immediately', 'mymail' ) ), __( 'after', 'mymail' ), ' <strong>' . $mymail_autoresponder_info['actions'][$autoresponder['action']]['label'] . '</strong>' );
		...
		case "total":
			...
			if ( !empty( $errors ) ) {
				echo '&nbsp;(<a href="edit.php?post_type=newsletter&page=mymail_subscribers&status=4" class="errors" title="' . sprintf( __( '%d emails have not been sent', 'mymail' ), $errors ) . '">+' . $errors . '</a>)';
			}
	...
	public function post_edit_assets() {
		...
		wp_localize_script( 'mymail-script', 'mymailL10n', array(
				...
				'save_template' => __( 'Save Template File', 'mymail' ),
				...
				'send_now' => __( 'Do you really like to send this campaign now?', 'mymail' ),
			) );
	...
	public function add_post_thumbnail_link( $content, $post_id ) {
		...
		if ( isset( $post ) && $post->post_type == 'newsletter' ) {
			...
			if ( in_array( $post->post_status, array( 'active', 'finished' ) ) || isset( $_GET['showstats'] ) ) {
				...
				$content .= '<p><label><input type="checkbox" name="auto_post_thumbnail" value="1" ' . checked( $meta, true, false ) . '> ' . __( 'Create Screenshot for Feature Image', 'mymail' ) . '</label></p>';

				$timestamp = wp_next_scheduled( 'mymail_auto_post_thumbnail', array( $post_id ) );
				if ( $timestamp + 2 >= time() ) {
					$content .= '<p class="description" title="' . __( 'Generating the screenshot may take a while. Please reload the page to update', 'mymail' ) . '"><span class="spinner"></span>' . __( 'Creating Screenshot', 'mymail' ) . '&hellip;</p>';
				}
	...
	public function save_campaign( $post_id, $post, $update = null ) {
		...
		if ( isset( $postdata ) ) {
			...
			if ( $meta['auto_post_thumbnail'] = isset( $_POST['auto_post_thumbnail'] ) ) {
				...
				wp_schedule_single_event( time(), 'mymail_auto_post_thumbnail', array( $post_id ) );

			} else {

				if ( $timestamp = wp_next_scheduled( 'mymail_auto_post_thumbnail', array( $post_id ) ) ) {
					wp_unschedule_event( $timestamp, 'mymail_auto_post_thumbnail', array( $post_id ) );
				}
	 ...
	 *
	 * @param unknown $id
	 * @param unknown $silence (optional)
	 * @return unknown
	 */
	...
	public function create_list_from_option( $name, $campaign_id, $option = 'open', $countonly = false ) {
		...
		if ( !current_user_can( 'mymail_edit_lists' ) ) {
			return false;
		}
	...
	public function get_recipients_part( $campaign_id, $parts = array( 'unopen', 'opens', 'clicks', 'unsubs', 'bounces' ), $page = 0, $orderby = 'sent', $order = 'ASC' ) {
		...
		if ( $count && $limit == $subscribers_count ) {
			$return .= '<tr ' . ( $i % 2 ? ' class="alternate" ' : '' ) . '><td colspan="7"><a class="load-more-receivers button aligncenter" data-page="' . ( $page + 1 ) . '" data-types="' . implode( ',', $parts ) . '" data-order="' . $order . '" data-orderby="' . $orderby . '">' . __( 'load more recipients from this campaign', 'mymail' ) . '</a>' . '<span class="spinner"></span></td></tr>';
		}
	...
	public function heartbeat( $response, $data ) {
		...
		case 'overview':
			...
			if ( !isset( $_POST['data']['mymail']['ids'] ) ) {
				break;
			}
	...
	public function send( $metadata, $userdata, $track = true, $force = false, $log = true ) {
		...
		if ( !$campaign || $campaign->post_type != 'newsletter' ) {
			return new WP_Error( 'wrong_post_type', __( 'wrong post type', 'mymail' ) );
		}

		if ( !in_array( $metadata->status, array( 0, 1, 2 ) ) && !$force ) {
			return new WP_Error( 'user_unsubscribed', __( 'User has not subscribed', 'mymail' ) );
		}
		...
		if ( $mail->sentlimitreached ) {
			return new WP_Error( 'sendlimit_reached', sprintf( __( 'Sent limit of %1$s reached! You have to wait %2$s before you can send more mails!', 'mymail' ), mymail_option( 'send_limit' ), human_time_diff( get_option( '_transient_timeout__mymail_send_period_timeout' ) ) ) );
		}
		...
		//  'X-MyMail: '.$metadata->hash,
		//  'X-MyMail-Campaign: '.$campaign->ID,
		//  'X-MyMail-ID: '.$MID,
		// ));
		...
		if ( $result && !is_wp_error( $result ) ) {
			if ( $log ) {
				do_action( 'mymail_send', $metadata->ID, $campaign->ID );
			}
		...
		if ( $mail->is_user_error() ) {
			if ( $log ) {
				do_action( 'mymail_subscriber_error', $metadata->ID, $campaign->ID, $mail->last_error->getMessage() );
			}
		...
		if ( $mail->last_error ) {
			if ( $log ) {
				do_action( 'mymail_campaign_error', $metadata->ID, $campaign->ID, $mail->last_error->getMessage() );
			}
	...
	public function check_for_autoresponder( $new_status, $old_status, $post ) {
		...
		if ( get_post_meta( $post->ID, 'mymail_ignore', true ) ) {
			return;
		}
	...
	public function get_post_thumbnail( $campaign_id ) {
		...
		if ( 307 == $code ) {
			wp_schedule_single_event( time() + 10, 'mymail_auto_post_thumbnail', array( $campaign_id ) );
		}
		...
		$filename = apply_filters( 'mymail_post_thumbnail_filename', 'newsletter-' . $campaign_id, $campaign ) . '.jpg';
		...
		$attachment = array(
			...
			'post_title' => apply_filters( 'mymail_post_thumbnail_title', $campaign->post_title, $campaign ),
			'post_content' => '',
			'post_status' => 'inherit',
		);
